    ...
    def __path_crawler(self):
        ...
        while True:
            if(self._city_com_queue.empty()):
                ...
                print('path {0[0]}: From {0[1]} to {0[2]} crawled...'.format(city_coms_data))
                url = 'http://api.map.baidu.com/direction/v1?mode=driving&origin={0[1]}&destination={0[2]}&origin_region={0[1]}&destination_region={0[2]}&output=json&ak=KtmshjiGv5nDDvYwcWbF0GIfhZf1anvE'.format(city_coms_data)
                ...
                while(timeout > 0):
                    ...
                    try:
                        ...
                        print('path {0[0]}: From {0[1]} to {0[2]} crawl succeed.'.format(city_coms_data))
                        break
        ...
        while not PARSER_EXIT_FLAG:
            ...
            try:
                ...
                while(timeout > 0):
                    ...
                    print('From {} to {} parse succeed: duration: {}, distance: {}'.format(result['origin'], result['destination'], result['driving_duration'], result['distance_km']))
                    
                    result_vector = (result['id'], result['origin'], result['destination'], result['driving_duration'], result['distance_km'], result['path'])
