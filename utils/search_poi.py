...
class KeywordSearcher(threading.Thread):
    ...
    # 关键字查找的源数据格式：id,keywords(多个关键字用竖线|分割),types(多个类型用竖线|分割),city(查询城市)
...
# 周边搜索
class AroundSearcher(threading.Thread):
    ...
    def run(self):
        print('No.{} AroundSearcher start...'.format(self._thread_id))
        self._search_poi()
        print('No.{} AroundSearcher finished!'.format(self._thread_id))

    # 周边搜索的源数据格式：id,keywords(多个关键字用竖线|分割),types(多个类型用竖线|分割),city(查询城市),location(中心点坐标，经度和纬度用,分割，经度在前，纬度在后),radius(查找半径，取值范围0-50000m),sortrule(返回结果排序规则，按距离排序：distance，综合排序：weight)
    def _search_poi(self):
        while True:
            if(self._search_queue.empty()):
                break
                ...
                url = 'http://restapi.amap.com/v3/place/around?&keywords={0[1]}&types={0[2]}&city={0[3]}&location={0[4]}&radius={0[5]}&sortrule={0[6]}&output=json&offset=20&key={key}&extensions=all'.format(
                    search_req, **self._search_params)
                        ...
                        for page in range(pages):
                            while True:
                                try:
                                    ...
                                    poi['city'] = search_req[3]
                                    poi['page_num'] = page_num
                                    poi['info'] = requests.get(
                                        sub_url, timeout=5).json()
                                    self._poi_queue.put(poi)
                                    print('id:{0}, page num{1} crawl succeed!'.format(
                                        poi['id'], page_num))
...
# 多边形搜索
class PolygonSearcher(threading.Thread):
    ...
    def run(self):
        print('No.{} PolygonSearcher start...'.format(self._thread_id))
        self._search_poi()
        print('No.{} PolygonSearcher finished!'.format(self._thread_id))

    # 多边形搜索的源数据格式：id,keywords(多个关键字用竖线|分割),types(多个类型用竖线|分割),city(城市),polygon(经纬度坐标对，经纬度用,分割，经度在前纬度在后，坐标对用|分割。多边形为矩形时可传入左上右下两顶点坐标对，其他情况下首尾坐标对需相同)
    def _search_poi(self):
        while True:
            if(self._search_queue.empty()):
                break
                ...
                url = 'http://restapi.amap.com/v3/place/polygon?&keywords={0[1]}&types={0[2]}&city={0[3]}&polygon={0[4]}&output=json&offset=20&key={key}&extensions=all'.format(
                    search_req, **self._search_params)
    ...
    def _parse_poi(self):
        ...
        while not PARSER_EXIT_FLAG:
            ...
            try:
                ...
                poi = self._poi_queue.get(True, 10)
                ...
                for i in range(num_of_pois):
                    ...
                    if(not result['poi_address']):
                        result['poi_address'] = 'null'
                ...
                print('Parse id {0} page {1} failed!'.format(poi['id'], poi['page_num']))
                print(parse_error)
...
def main():
    ...
    with poi_db:
        poi_db.execute(
            'create table poi(id int, keywords varchar(255), types varchar(255), city varchar(20), page_num varchar(20), poi_id varchar(20), poi_name nvarchar(20), poi_type varchar(50), poi_typecode varchar(20), poi_address nvarchar(255), poi_loc varchar(20))')

        with open(parse_error_file, mode='w', encoding='utf-8') as f_pares_error:

            if(int(search_type) == 1):
                search_threads = []
                search_list = ['search_thread' + str(num) for num in range(5)]
